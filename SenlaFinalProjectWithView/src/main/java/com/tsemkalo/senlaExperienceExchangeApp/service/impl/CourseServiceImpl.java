...
public class CourseServiceImpl extends AbstractServiceImpl<Course, CourseDao> implements CourseService {
	...
	private void editCourseTeacher(String currentUsername, Course newCourse, Course course) {
		...
		if (RoleType.ADMIN.equals(teacher.getRole().getName())) {
			...
			for (Lesson lesson : course.getLessons()) {
				Lesson scheduleOverlap = userDao.teacherScheduleOverlapsPresent(newCourse.getTeacher().getUsername(), lesson.getLessonDate(), lesson.getDuration());
	...
	@Override
	public Boolean isSubscribed(String studentUsername, Long courseId) {
		Long studentId = userDao.getUserByUsername(studentUsername).getId();
		Course course = courseDao.getById(courseId);
		for (Lesson lesson : course.getLessons()) {
			if (subscriptionDao.getSubscription(studentId, lesson.getId()) != null) {
				return true;
			}
		}
		return false;
