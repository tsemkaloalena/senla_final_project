...
public class LessonServiceImpl extends AbstractServiceImpl<Lesson, LessonDao> implements LessonService {
	...
	public Lesson editLesson(String currentUsername, Lesson newLesson) {
		...
		if (newLesson.getDuration() != null) {
			lesson.setDuration(newLesson.getDuration());
		}
		if (lesson.getCourse() != null) {
			checkCourseAccordingToLesson(currentUsername, lesson);
			courseDao.updateCourseAccordingToLesson(lesson);
	...
	private void editTeacher(String currentUsername, Long lessonId, Lesson newLesson) {
		...
		if (RoleType.ADMIN.equals(teacher.getRole().getName())) {
			...
			Lesson scheduleOverlap = userDao.teacherScheduleOverlapsPresent(newLesson.getTeacher().getUsername(), newLesson.getLessonDate(), newLesson.getDuration());
			if (scheduleOverlap != null) {
				throw new IncorrectDataException("This teacher already has a lesson from " + scheduleOverlap.getLessonDate() +
						" to " + scheduleOverlap.getLessonDate().plusMinutes(scheduleOverlap.getDuration()));
			}
	...
	public Boolean subscribe(String currentUsername, Long lessonId) {
		...
		Lesson scheduleOverlap = userDao.studentScheduleOverlapsPresent(currentUsername, lesson.getLessonDate(), lesson.getDuration());
		if (scheduleOverlap != null) {
	...
	@Override
	public Boolean isSubscribed(String studentUsername, Long lessonId) {
		Long studentId = userDao.getUserByUsername(studentUsername).getId();
		return subscriptionDao.getSubscription(studentId, lessonId) != null;
	}
