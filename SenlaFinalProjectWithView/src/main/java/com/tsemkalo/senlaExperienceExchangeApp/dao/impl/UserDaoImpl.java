...
public class UserDaoImpl extends AbstractDaoImpl<User> implements UserDao {
	...
	@Override
	public Lesson studentScheduleOverlapsPresent(String username, LocalDateTime start, Integer duration) {
		...
		for (Subscription subscription : student.getSubscriptions()) {
			...
			LocalDateTime subscriptionFinish = subscription.getLesson().getLessonDate().plusMinutes(subscription.getLesson().getDuration());
	...
	@Override
	public Lesson teacherScheduleOverlapsPresent(String username, LocalDateTime start, Integer duration) {
		...
		for (Lesson lesson : getTeacherLessons(teacher.getId())) {
			...
			LocalDateTime subscriptionFinish = lesson.getLessonDate().plusMinutes(lesson.getDuration());
			if (start.isAfter(subscriptionStart) && start.isBefore(subscriptionFinish) ||
					finish.isAfter(subscriptionStart) && finish.isBefore(subscriptionFinish)) {
				return lesson;
			}
